...
class ExceptionHandler(program: Program, binding: ActivityMainBinding, ) {
    ...
    fun throwRuntime(msg: String, context: Context = binding.rootLayout.context, showLine: Boolean = true){
        program.stopExecution()
        val intent: Intent = Intent(context, Error::class.java)
        intent.putExtra(EXC_MSG, "${if(showLine) program.getCurrentLine().toString()+": " else ""}$msg");
